    ...
    try {
        if (((referenceMessage?.author.bot || message.channel.id === process.env.GARDEN_ID || message.content.toLowerCase().includes("mods") || message.content.includes("ローゼミ") || message.content.includes("ロゼミ") || message.content.includes("ロセミ") || /\bRosemi\b/i.test(message.content)) && message.channel.id !== process.env.COMMANDS_ID && !message.author.bot && !message.content.startsWith('!') && !message.content.startsWith(':')) || (newCount === 0 && message.channel.id === process.env.GENERAL_ID)) {
            ...
            prevMessages.forEach((msg) => {
                checkCount++;
                if (msg.content.startsWith('!') || msg.content.startsWith('<') || msg.content.startsWith('||') || msg.content.length <= 0)
                    return;
                ...
                if (!initialIsCapital(msg.author.username))
                    usernem = capitalizeFirstLetter(msg.author.username);
                if (msg.author.username === "RosemiBot")
                    chatinput += `${botStart}Rosemi: ${msg.content}\n${mlEnd}`;
                else if (msg.author.username === "aso19")
                    chatinput += `${userStart}Papa: ${usercontent}\n${mlEnd}`;
                else
                    chatinput += `${userStart}${usernem}: ${usercontent}\n${mlEnd}`;
                if (msg.content.startsWith("=refresh") || msg.content.startsWith("=reset"))
                    chatinput = injection;
            ...
            nSplitMessage.forEach((msg) => {
                if (tempMessage.length >= 1500) { // If message is close to going over the 2k character threshold
