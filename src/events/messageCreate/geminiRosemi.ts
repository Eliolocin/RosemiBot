...
function trimMEND(text: string): string {
...
function trimROSE(text: string, botName: string): string {
  ...
  try {
    ...
    if (
      ...
      if (botData.botDatabase && botData.botDatabase.length > 0) {
        injection += botData.botDatabase.map(item => `[${item}]`).join("\n") + "\n";
    }
      ...
      if (
        ...
        botData.conversationExamples.forEach((example) => {
            const userInput = example.input.replace('{user}', userData.nickname);
          const botOutput = example.output.replace('{bot}', botData.botName);
          injection +=
      ...
      chunkedMessage.forEach((chunk) => {
        if (chunk.length > 5) {
          message.channel.send(trimROSE(trimMEND(chunk), botData.botName));
        }
      });
    }
  } catch (err) {
